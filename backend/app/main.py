...
from app.engine.connector import (
    ...
    upload_dataframe_to_postgres_test_db,
    upload_dataframe_to_mysql_test_db,
...
class SnowflakeExportReq(BaseModel):
    account: str
    user: str
    password: str
    warehouse: Optional[str] = "COMPUTE_WH"
    database: Optional[str] = "TEST_DATA_DB"
    db_schema_name: Optional[str] = Field(default="PUBLIC", alias="schema")
...
class PostgresExportReq(BaseModel):
...
class S3CreateBucketReq(S3BucketsReq):
    bucket_name: str
...
async def ingest_file(file: UploadFile = File(...), override_name: Optional[str] = None):
    try:
        ...
        return {"status": "success", "dataset_name": dataset_name, "rows": df.height, "columns": df.columns}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))

@app.post("/api/ingest/url")
async def ingest_url(req: UrlFetchReq):
    try:
        ...
        if b"<!doctype html>" in content[:200].lower() or b"<html" in content[:200].lower():
            raise HTTPException(status_code=400, detail="Provided URL returned a webpage viewer instead of raw data. Verify sharing permissions.")

        parsed_filename = req.custom_dataset_name or clean_url.split("/")[-1].split("?")[0] or "url_dataset.csv"
        if "." not in parsed_filename:
            parsed_filename += ".csv"

        dataset_name = clean_filename(os.path.splitext(parsed_filename)[0])
        df = load_file_to_dataframe(content, parsed_filename)
        save_df_to_parquet(df, dataset_name)
        return {"status": "success", "dataset_name": dataset_name, "rows": df.height, "columns": df.columns}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
@app.post("/api/connect/postgres/schemas")
async def get_schemas(req: GenericSchemaReq):
    try:
        ...
        if data.get("account") or data.get("warehouse") or data.get("driver") == "snowflake":
            ...
            data["driver"] = "postgresql"
        config = build_config_from_dict(data)
        if config.driver == "snowflake":
            schemas = get_snowflake_schemas(config)
        elif config.driver == "postgresql":
            schemas = get_postgres_schemas(config)
        else:
            schemas = ["PUBLIC"]
...
@app.post("/api/connect/postgres/tables")
async def get_tables(req: GenericTableReq):
    try:
        ...
        config = build_config_from_dict(data)
        if config.driver == "snowflake":
            tables = get_snowflake_tables(config)
        elif config.driver == "postgresql":
            tables = get_postgres_tables(config)
        else:
...
@app.post("/api/connect/postgres/import")
async def import_table(req: GenericImportReq):
    try:
        ...
        schema_name = config.schema or ("PUBLIC" if config.driver == "snowflake" else "public")
        ...
        if action == "extract_test_db":
            ...
            if config.driver == "snowflake":
                success, msg = upload_dataframe_to_snowflake_test_db(
                    config=config, original_database=database_name, schema_name=schema_name,
                    table_name=table_name, dataframe_dicts=dataframe_dicts, force_external_schema=force_external_schema
                )
            elif config.driver == "postgresql":
                success, msg = upload_dataframe_to_postgres_test_db(
                    config=config, schema_name=schema_name, table_name=table_name,
                    dataframe_dicts=dataframe_dicts, force_external_schema=force_external_schema
                )
            else:
                success, msg = upload_dataframe_to_mysql_test_db(
                    config=config, table_name=table_name, dataframe_dicts=dataframe_dicts
                )
...
@app.post("/api/export/snowflake")
async def export_to_snowflake(req: SnowflakeExportReq):
    try:
        ...
        config = DBConfig(
            driver="snowflake", account=req.account, user=req.user, password=req.password,
            warehouse=req.warehouse or "COMPUTE_WH", database=req.database or "TEST_DATA_DB", schema=req.db_schema_name or "PUBLIC"
        )
        success, msg = upload_dataframe_to_snowflake_test_db(
            config=config, original_database=req.database, schema_name=req.db_schema_name,
            table_name=req.tableName, dataframe_dicts=df.to_dicts(), force_external_schema=req.force_external_schema
        )
        return {"status": "success", "message": msg}
    except Exception as e:
        traceback.print_exc()
...
@app.post("/api/export/postgres")
async def export_to_postgres(req: PostgresExportReq):
    try:
        parquet_path = get_parquet_path(req.dataset_name)
        if not parquet_path.exists():
            raise HTTPException(status_code=404, detail="Dataset not found in workspace.")
        ...
        config = DBConfig(
            driver="postgresql", host=req.host or "localhost", port=req.port or 5432,
            user=req.user, password=req.password, database=req.database or "test_db", schema=req.db_schema_name or "public"
        )
        success, msg = upload_dataframe_to_postgres_test_db(
            config=config, schema_name=req.db_schema_name, table_name=req.tableName,
            dataframe_dicts=df.to_dicts(), force_external_schema=req.force_external_schema
        )
        return {"status": "success", "message": msg}
    except Exception as e:
        traceback.print_exc()
...
@app.post("/api/export/mysql")
async def export_to_mysql(req: MySqlExportReq):
    try:
        parquet_path = get_parquet_path(req.dataset_name)
        if not parquet_path.exists():
            raise HTTPException(status_code=404, detail="Dataset not found in workspace.")
        ...
        config = DBConfig(
            driver="mysql", host=req.host or "localhost", port=req.port or 3306,
            user=req.user, password=req.password, database=req.database or "test_db"
        )
        success, msg = upload_dataframe_to_mysql_test_db(
            config=config, table_name=req.tableName, dataframe_dicts=df.to_dicts()
        )
        return {"status": "success", "message": msg}
    except Exception as e:
        traceback.print_exc()
...
async def ingest_from_s3(req: S3FetchReq):
    try:
        df = fetch_dataset_from_s3(bucket=req.bucket, key=req.key, aws_access_key_id=req.aws_access_key_id, aws_secret_access_key=req.aws_secret_access_key, region_name=req.region_name)
...
async def get_s3_buckets(req: S3BucketsReq):
    try:
        return list_s3_buckets_and_folders(aws_access_key_id=req.aws_access_key_id, aws_secret_access_key=req.aws_secret_access_key, region_name=req.region_name)
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))

@app.post("/api/ingest/s3/create-bucket")
async def create_s3_bucket(req: S3CreateBucketReq):
    try:
        create_s3_bucket_if_not_exists(req.bucket_name, req.aws_access_key_id, req.aws_secret_access_key, req.region_name)
        return {"status": "success", "message": f"Bucket '{req.bucket_name}' created successfully."}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def get_s3_folders(req: S3FoldersReq):
    try:
        return list_s3_buckets_and_folders(aws_access_key_id=req.aws_access_key_id, aws_secret_access_key=req.aws_secret_access_key, region_name=req.region_name, bucket=req.bucket)
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))

@app.post("/api/ingest/s3/objects")
async def get_s3_objects(req: S3ObjectsReq):
    try:
        s3 = get_s3_client(req.aws_access_key_id, req.aws_secret_access_key, req.region_name)
        response = s3.list_objects_v2(Bucket=req.bucket, Prefix=req.prefix or "", Delimiter='/')
        
        folders = [p.get('Prefix') for p in response.get('CommonPrefixes', []) if p.get('Prefix')]
        files = [content.get('Key') for content in response.get('Contents', []) if content.get('Key') != req.prefix]
...
async def export_to_s3(req: S3ExportReq):
    try:
        ...
        export_dataset_to_s3(df=df, bucket=req.bucket, destination_key=req.destination_key, format=req.format, aws_access_key_id=req.aws_access_key_id, aws_secret_access_key=req.aws_secret_access_key, region_name=req.region_name)
        return {"status": "success", "message": f"Successfully exported to s3://{req.bucket}/{req.destination_key}"}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
